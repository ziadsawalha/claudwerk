...
const debugControlSend: MessageHandler = (ctx, data) => {
  ...
  if (!conversationId || !channel || !command) {
    if (conversationId)
      emitResult(ctx, conversationId, traceId, channel, command, {
        ok: false,
        code: 'bad_request',
  ...
  if (spec?.danger && !isAdminCaller(ctx, conv.project)) {
    emitResult(ctx, conversationId, traceId, channel, command, {
      ok: false,
      code: 'forbidden',
      error: `danger command ${command} requires admin`,
    })
    return
  }
  ...
  ctx.log.info(
    `[debug-control] ${who} -> ${conversationId.slice(0, 8)} ${channel}:${command}${spec?.danger ? ' [DANGER]' : ''}`,
  )
  ...
  if (channel === 'cc_control' && isDaemon) {
    ...
    emitResult(ctx, conversationId, traceId, channel, command, {
      ...
      error: 'cc_control is not reachable on a daemon conversation (worker control channel is internal)',
    })
    return
  }
  if (channel === 'daemon_op' && !isDaemon) {
    ...
    emitResult(ctx, conversationId, traceId, channel, command, {
      ok: false,
      code: 'unsupported_transport',
      error: 'daemon_op is only available on a daemon conversation',
    })
    return
  }
  if (channel === 'daemon_op') {
    ...
    emitResult(ctx, conversationId, traceId, channel, command, {
      ...
      error: 'daemon_op dispatch not wired yet (next increment)',
    })
    return
  }
  ...
  if (!ws) {
    ...
    emitResult(ctx, conversationId, traceId, channel, command, {
      ok: false,
      code: 'no_agent_host',
      error: 'conversation has no live agent host',
    })
    return
  }
